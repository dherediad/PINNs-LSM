...
nombre = 'vortex_100x100_5c_200n_sigmoid'

# Crear carpeta para guardar modelos y resultados (carpeta principal)
nombre_carpeta_principal = f'experimentos/vortex/{nombre}/'
...
def pde_func(x,y,t,net):
    ...
    pdex = -(torch.sin(np.pi*x)**2)*torch.sin(2*np.pi*y)*torch.cos(np.pi*t/1)*8
    pdey = torch.sin(2*np.pi*x)*(torch.sin(np.pi*y)**2)*torch.cos(np.pi*t/1)*8
    pde = u_t - (pdex*u_x + pdey*u_y)
